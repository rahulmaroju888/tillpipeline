...
export const jobLatest = (jobId) => async (dispatch) => {
  dispatch({ type: JOB_LATEST_REQUEST, payload: jobId });
  try {
    const { data } = await Axios.get(`http://10.1.2.4:5001/job/${jobId}/`);
    dispatch({ type: JOB_LATEST_SUCCESS, payload: data });
  } catch (error) {
    dispatch({
      type: JOB_LATEST_FAIL,
      payload:
        error.response && error.response.data.message
          ? error.response.data.message
          : error.message,
