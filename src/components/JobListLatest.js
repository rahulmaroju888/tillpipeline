...
export default function JobList(props) {
  ...
  // const productDetails = useSelector((state) => state.JobList);
  // const { loading, error, product } = jobDetailsReducer;

  // useEffect(() => {
  //   dispatch(jobListReducer(jobLatest));
  // }, [dispatch, jobLatest]);
  return (
        ...
        <div className="card-body">
          ...
          <div className="table-responsive mt-3 onclickJoblistTable">
            <table className="table table-striped table-hover">
              ...
              <tbody>
                {
                  props?.tableData?.map(item => <Job key={item.id} job={item} />)}
              </tbody>
            </table>
          </div>
        </div>
